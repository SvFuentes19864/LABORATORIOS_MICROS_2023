...
void saveToEEPROM(){
    ...
    PORTD = EEDATA;          // Mostrar el dato leído en el puerto D
...
void __interrupt() isr(void)
{
    if(INTCONbits.RBIF)             // Si la interrupción proviene del pin RB0
    {
        ...
        if(!PORTBbits.RB2){         // Verificamos si el botón de guardar está presionado
            
            Bandera = 1;            // Apagamos la bandera del while
            saveToEEPROM();         // Guardamos el valor del ADC en la EEPROM
        }
        if(!PORTBbits.RB3){         // Verificamos si el botón de guardar está presionado
            
            readToEEPROM();         // Leemos el valor guardado en la EEPROM
        }
    }
}
...
//________________________CONFIGURACIÓN DE PUERTOS______________________________
    ...
    TRISB = 0b00001111;                 // Puerto B como entradas para los botones
